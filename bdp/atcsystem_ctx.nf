...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(atcsystem_ctx))==(source/=destination)
END
&
...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(atcsystem_ctx))==(mapw,maph,aircraftid,source,destination,speed_limit,altitude_limit);
  Inherited_List_Constants(Machine(atcsystem_ctx))==(?);
  List_Constants(Machine(atcsystem_ctx))==(mapw,maph,aircraftid,source,destination,speed_limit,altitude_limit)
END
&
THEORY ListSetsX IS
  ...
  List_Valuable_Sets(Machine(atcsystem_ctx))==(AIRCRAFTS);
  ...
  List_Defered(Machine(atcsystem_ctx))==(AIRCRAFTS);
  List_Sets(Machine(atcsystem_ctx))==(DIRECTION,STATE,AIRCRAFTS);
  Set_Definition(Machine(atcsystem_ctx),STATE)==({CLEAR,OCCUPIED});
  Set_Definition(Machine(atcsystem_ctx),AIRCRAFTS)==(?)
END
&
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(atcsystem_ctx))==(mapw = 3 & maph = 3 & aircraftid: NAT & speed_limit = 2 & altitude_limit: NAT & source: struct(xx>>(0..3),yy>>(0..3)) & destination: struct(xx>>(0..3),yy>>(0..3))-{source} & AIRCRAFTS: FIN(INTEGER) & not(AIRCRAFTS = {}) & DIRECTION: FIN(INTEGER) & not(DIRECTION = {}) & STATE: FIN(INTEGER) & not(STATE = {}))
END
&
...
&
THEORY SetsEnvX IS
  Sets(Machine(atcsystem_ctx)) == (Type(DIRECTION) == Cst(SetOf(etype(DIRECTION,0,3)));Type(STATE) == Cst(SetOf(etype(STATE,0,1)));Type(AIRCRAFTS) == Cst(SetOf(atype(AIRCRAFTS,"[AIRCRAFTS","]AIRCRAFTS"))))
END
&
THEORY ConstantsEnvX IS
  Constants(Machine(atcsystem_ctx)) == (Type(NN) == Cst(etype(DIRECTION,0,3));Type(SS) == Cst(etype(DIRECTION,0,3));Type(EE) == Cst(etype(DIRECTION,0,3));Type(WW) == Cst(etype(DIRECTION,0,3));Type(CLEAR) == Cst(etype(STATE,0,1));Type(OCCUPIED) == Cst(etype(STATE,0,1));Type(mapw) == Cst(btype(INTEGER,?,?));Type(maph) == Cst(btype(INTEGER,?,?));Type(aircraftid) == Cst(btype(INTEGER,?,?));Type(source) == Cst(rtype((xx: btype(INTEGER,0,3)),yy: btype(INTEGER,0,3)));Type(destination) == Cst(rtype((xx: btype(INTEGER,?,?)),yy: btype(INTEGER,?,?)));Type(speed_limit) == Cst(btype(INTEGER,?,?));Type(altitude_limit) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
